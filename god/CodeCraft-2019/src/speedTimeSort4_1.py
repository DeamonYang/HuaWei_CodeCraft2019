...
def time_split(group, car_per_sec):
    ...
    for i in range(batch_num):
        group_divide_time2.append([])
    y = 0
    for i in group_divide_time:#一组
        x = -1
        ...
        for j in i:#一辆车
            ...
            if j != []:
                ...
                x1, x2 = 0, 0
                y1, y2 = 1, 1
                for m in range(Car.destination[j-Car.id[0]]):
                    x1 += 1
                    if (x1 > 8):
                        x1 = 1
                        y1 += 1
                for n in range(Car.origin[j-Car.id[0]]):
                    x2 += 1
                    if (x2 > 8):
                        x2 = 1
                        y2 += 1
            ...
            #print(x)
                car_length[x] = [Car.id[j-Car.id[0]], tmp]
        car_length = sorted(car_length, key=lambda x: x[1])#260ge
        #print(car_length)
        for i in car_length:
            group_divide_time2[y].append(i[0])
        y+=1
        car_length = []
    #print(group_divide_time2)

    return group_divide_time2
