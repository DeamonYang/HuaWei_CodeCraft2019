...
def speedSort(Car_count, Car_speed, Car_id):
    ...
    speed_dic = {}
    for i in range(max_speed*max_time):#数组变成字典--car.speed：[car.id]
    #for i in range(max_speed):
        speed_dic[i+1] = car_divide_time[i]
    #print(speed_dic)
    return speed_dic
...
def update_loss(array_loss, array_dis, Road_count, Road_length, Road_channel, Road_speed, Road_isDuplex, Road_roadFrom, Road_roadTo):
    channel_count = 0
...
def cal(plan_roadLength, plan_road, road_loss):
    ...
    car_per_sec = 240  #260/5溢出 #250反向排出发时间反而效果好 240/7/664/505
    interval_time = 7
    time = 1
    n = 0
    answer = []
