...
def time_split(group, car_per_sec):
    ...
    #group_position = []
    #if speed in [1, 2]:
    #    car_per_batch = car_per_sec
    #elif speed in [3, 4]:
    #    car_per_batch = int(car_per_sec * 1.1)
    #elif speed in [5, 6]:
    #    car_per_batch = int(car_per_sec * 1.1)
    #else:
    #    car_per_batch = int(car_per_sec * 1.1)
...
def update_loss(array_loss, array_dis, Road_count, Road_length, Road_channel, Road_speed, Road_isDuplex,
    ...
    for i in range(Road_count):
        ...
        loss = Road_length[i] * (1 / min(Road_speed[i], speed) + 50 * use_rate / Road.channel[i])\
               + 0.3 * max(cross_loss[Road_roadFrom[i]+1], cross_loss[Road_roadTo[i]+1])#80反升高
        #else:
            #loss = Road_length[i] * (1 + channel / Road_channel[i])
        #print(loss, Road_length[i])
...
def cal(plan_roadLength, plan_road, road_loss):
    ...
    #road_path = '../config/road.txt'#sys.argv[2]
    #cross_path = '../config/cross.txt'#sys.argv[3]
    ...
    for speed in speed_list:
        ...
        if not cur_group:#空组
            continue
        car_per_sec = 25#25 #20/610 32溢出 对比一下870的
        car_in_map = 2000
        ...
        for batch in group_divide_time:
            ...
            road_loss = update_loss(road_loss, plan_roadLength, Road.count, Road.length, Road.channel, Road.speed,
                                    Road.isDuplex, Road.roadFrom, Road.roadTo, road_percent_list, speed, cross_loss)
