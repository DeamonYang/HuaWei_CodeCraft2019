...
CARNAMESPACE,ROADNAMESPACE,CROSSNAMESPACE = [],[],[] #只是id
...
class CAR(object):
    def __init__(self,id_,from_,to_,speed_,planTime_):
        ...
        self.carColor = [int(value) for value in np.random.random_integers(0, 255, [3])]#不用考虑
        # **** dynamic parameters ****#
...
def main():
    ...
    count = 0
    for i,line in enumerate(answerInfo): #i相当于id，line相当于answer的所所有信息
        #*********************************数据初始化**********************************#
        if line.__len__() <3:
            continue
        if line[0]=='#':
        ...
        #*******************************车辆初始化**************************************#
        CARDICT[carId].simulateInit(planTime_,route) #开始时间和路径存储
        count+=1

    print("There are %d cars' route preinstalled"%count)
    CARDISTRIBUTION[0] = CARNAMESPACE.__len__()#车辆总数
